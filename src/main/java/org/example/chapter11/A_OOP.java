...
// >> 하나의 클래스가 가지는 하나의 역할과 관련된 변경 사항만 클래스에 영향을 미쳐야 함
...
// 2) 올바른 설계 예시 - 각각의 클래스가 자신만의 역할을 담당 (유지보수와 테스트가 쉬워짐)
class Janitor {
    void cleanClassroom() {
        System.out.println("교실을 청소합니다.");
    }
...
// 1) 잘못된 설계 예시 - 새로운 학생 유형이 생길 때마다 if문 추가
// class LunchMenu {
//      void serveLunch(String studentType) {
//          if (studentType.equals("일반 학생")) {
//             System.out.println("알레르기가 없습니다");
//         }  if else (studentType.equals("땅콩 알레르기")) {
//
//             System.out.println("알레르기가 있습니다.");
//         }
//     }
// }
...
class AllergyStudent implements LunchMenuInterface {
    @Override
    public void serveLunch() {
        System.out.println("알레르기 학생 - 특벽ㄹ 배식구로 이동하세요");
    }
}
...
// : 상위 클래스의 객체를 하위 클래스의 객체로 피환하더라도 프로그램의 동작이 일관되게 유지되어야 함
