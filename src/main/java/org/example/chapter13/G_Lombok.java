...
// === 롬복(Lombok) 라이브러리 === //
// : 자바에서 반복적으로 작성해야 하는 보일러 플레이트(BoilerPlate) 코드를 줄여 주는 라이브러리
// - 어노테이션(코드 주석)을 사용하여 코드를 간결하게 만들어 주는 역할
...
// == 롬복(Lombok) 환경 설정 == //
// cf) Java + Gradle 빌드 도구 사용 프로젝트

// 1) build.gradle 파일에 롬복 추가

// +) Maven Repository
// : Java 프로젝트에 필요한 라이브러리(외부 코드)를 쉽게 다운로드하고 관리할 수 있도록 도와주는 저장소
// - 주로 Maven 빌드 도구와 함께 사용되지만
//      , Gradle도 함께 사용
...
// 2) @AllArgsConstructor: 모든 플드값을 인자로 가지는 생성자
...
// : 두 객테의 내용이 같은지(동등성) 비교 & 두 객체가 같은 객체인지(동일성) 비교 연산자
// >> boolean 값을 반환

// 5. @Builder 어노테이션
// : 복잡한 객체 생성 시 사용하는 빌더 패턴을 자동으로 구현
// - 필드값을 모두 메서드 체이닝으로 가져옴 (필수값 또한 메서드 체인으로 구성)
// - 메서드 체이닝 이름은 필드명
// >> 필수값을 체이닝에서 누락 시 오류
...
public class G_Lombok {
    public static void main(String[] args) {
        LombokClass lombokClass1 = new LombokClass("자바는 재미있어", "이승아", "코리아");
