...
class Book {
    ...
    void displayInfo() {
        ...
        return;
        // System.out.println("실행 안 됨"); - 해당 코드에 접근 불가
    }

    // 클래스 생성자
    // : 반드시 하나의 class 내부에는 한 개 이상의 생성자가 존재
    // - new 연산자를 사용하여 호출될 메서드
    // - 필드값 촉화에 주로 사용
    //  >> 생략 시 속성에 기본값 할당: int(0), boolean(false), char(''), 참조 타입(null)

    // +) 사용자 정의 생성자 메서드 생략 시
    //  >> 클래스명과 동일하면서 '매개 변수'와 '동작'이 없는 메서드가 생략
    Book () {}
...
public class D_Object {
    public static void main(String[] args) {
        ...
        myBook3.displayInfo(); // 신데렐라 에이원출판사

        myBook1.title = "개구리왕자";
        myBook1.publisher = "개굴출판사";
        myBook1.displayInfo(); // 개구리왕자 개굴 출판사
