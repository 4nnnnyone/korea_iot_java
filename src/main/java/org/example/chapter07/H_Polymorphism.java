...
class Vehicle {
    void depart() { // depart : 출발하다
        System.out.println("운송 수단이 출발합니다.");
    }
}
...
class Bus extends Vehicle {
    @Override
    void depart() {
        System.out.println("버스가 출발합니다. 자리에 앉아 주세요.");
    }
}
...
class Subway extends Vehicle {
    @Override
    void depart() {
        System.out.println("문이 닫힙니다. 열차가 곧 출발합니다.");
    }
}
...
public class H_Polymorphism {
    public static void main(String[] args) {
        ...
        // >> boolean 타입으로 반환
        // 업캐스팅된 객체를 다운캐스팅할 경우 instanceof 클래스 타입 객체가 맞는지 확인 후 캐스팅

        if (subway instanceof Vehicle) {
            // subway 객체가 Vehicle 인스턴스 || Vehicle을 상속받은 인스턴스일 경우
            subway.depart(); // 문이 닫힙니다. 열차가 곧 출발합니다.
        }

        if (bus instanceof Vehicle) {
            bus.depart(); // 버스가 출발합니다. 자리에 앉아 주세요.
        }

        if (vehicle instanceof Bus) {
            System.out.println("운송 수단은 버스입니다.");
        } else {
            System.out.println("운송 수단은 버스가 아닙니다."); // 운송 수단은 버스가 아닙니다.
        }
